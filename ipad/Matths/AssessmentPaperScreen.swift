...
struct AssessmentPaperScreen: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let attempt = store.currentAttempt {
                ...
                ContentUnavailableView("응시 정보를 찾을 수 없습니다", systemImage: "doc.questionmark")
            }
        }
        ...
        .onReceive(Timer.publish(every: 1, on: .main, in: .common).autoconnect()) { _ in
            ...
            if a.remainingSeconds(monotonicElapsed: Double(timer.elapsedMs) / 1000) <= 0 { submit() }
        }
        .confirmationDialog("아직 답하지 않은 문항이 있습니다. 제출할까요?",
    ...
    private var header: some View {
        HStack(spacing: Tokens.Space.s4) {
            ...
            HStack(spacing: 2) {
                ...
                if let a = store.currentAttempt, a.submittedAt == nil {
                    let left = max(0, a.remainingSeconds(monotonicElapsed: Double(timer.elapsedMs) / 1000))
                    Text(String(format: "%d:%02d", left / 60, left % 60))
                        .font(.mNumeric).monospacedDigit()
                        .foregroundStyle(left <= 60 ? Tokens.danger : Tokens.ink)
                } else {
                    Text(timer.display).font(.mNumeric).foregroundStyle(Tokens.ink).monospacedDigit()
                }
            }
        }
    ...
    @ViewBuilder private func submitRow(_ attempt: AssessmentAttemptV2) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s2) {
            Button("제출하고 채점받기") {
                if answeredCount(attempt) < attempt.questions.count { confirmSubmit = true }
                else { submit() }
            }
            .buttonStyle(PrimaryButtonStyle())
            .frame(maxWidth: 280)
    ...
    private func submit() {
        timer.pause()
        store.submitPaper(monotonicElapsed: Double(timer.elapsedMs) / 1000)
    }
    ...
    private func resultCard(_ a: AssessmentAttemptV2) -> some View {
        ...
        return VStack(alignment: .leading, spacing: Tokens.Space.s3) {
            ...
            if a.disqualified == true {
                Label("시간 초과 · 실격 처리", systemImage: "clock.badge.xmark")
            ...
            Text(passed
                 ? "\(AssessCatalog.data.passScore)점 이상 — 다음 단계 평가가 열립니다."
                 : "\(AssessCatalog.data.passScore)점 미만 — 아래 해설을 확인하고 새 회차로 재응시하세요. 수치와 발문은 매회 바뀝니다.")
...
struct PaperWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        config.userContentController.addUserScript(WKUserScript(
            source: Self.payloadJS(attempt: attempt),
    ...
    private static func payloadJS(attempt: AssessmentAttemptV2) -> String {
        ...
        return "window.MATTHS_PAPER = \(json);"
    }
