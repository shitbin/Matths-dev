...
struct GoatArenaRulebookScreen: View {
    ...
    private func offlineRulebookUnavailable(_ mode: String) -> some View {
        RulebookSection(
            ...
            summary: "정산·자격 규칙의 오래된 복사본을 표시하지 않습니다. 네트워크 연결 뒤 다시 열어 현재 서버 적용본을 확인해주세요."
        ) {
        ...
        Group {
            RulebookSection(number: "F1", title: "Final Ranking", summary: "현재 공식 입력으로 매번 다시 계산하며 MMR이나 Arena를 바꾸지 않습니다.") {
                ...
                RulebookBullets(items: [
                    ...
                    "동점은 Final Rating → 시즌 정산 일반 공격 횟수 → Skill MMR → 안정 사용자 ID 순서입니다."
                ])
            }
            ...
            Text("서버의 활성 정책과 경기 원장이 최종 판정 기준입니다.")
                .font(.mCaption)
                .foregroundStyle(Tokens.text3)
    ...
    var body: some View {
        RulebookSection(
            ...
            RulebookTable(headers: ["항목", "현재 값"], rows: [
                ...
                ["최소 유료 일반 공격", "\(policy.minimumPaidNormalAttacks)회"],
                ["최소 페이백 점수", "\(policy.minimumScoreDays)점"],
            ])
