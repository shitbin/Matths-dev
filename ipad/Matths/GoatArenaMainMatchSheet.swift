...
struct GoatArenaMainMatchSheet: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: Tokens.Space.s7) {
                    intro
                    if isLoading && pendingCommand == nil {
                        loadingView
                    }
                    if let pendingCommand {
                        pendingNotice(pendingCommand)
                        if let selectedTarget {
                            submitButton(selectedTarget)
                        }
                    } else if let options {
                        summary(options)
            ...
            .confirmationDialog(
                ...
                if let invitation = cancellationCandidate {
                    Text("\(invitation.targetTier) 초대를 취소하면 예약한 \(invitation.reservedLearningDays)일이 모두 사용 가능 학습일로 돌아옵니다.")
                }
            }
        }
    }
        ...
        Group {
            if compact || dynamicTypeSize.isAccessibilitySize {
                VStack(spacing: Tokens.Space.s3) {
                    summaryItem("현재 티어", options.currentTier ?? "확인 중")
                ...
                ForEach(invitations) { invitation in
                    HStack(alignment: .center, spacing: Tokens.Space.s3) {
                        VStack(alignment: .leading, spacing: Tokens.Space.s1) {
                            Text("\(invitation.targetTier) · \(invitation.stakeDays)일 예약")
                                .font(.mBodyB)
                                .foregroundStyle(Tokens.text1)
                        ...
                        HStack(spacing: Tokens.Space.s3) {
                            VStack(alignment: .leading, spacing: Tokens.Space.s1) {
                                Text(target.tier)
                                    .font(.mBodyB)
                                    .foregroundStyle(Tokens.text1)
                                Text("\(target.gap)단계 차이 · \(target.minimumStakeDays)~\(target.maximumStakeDays)일")
                                    .font(.mCaption)
                                    .foregroundStyle(Tokens.text2)
                            }
                            Spacer(minLength: Tokens.Space.s3)
                            ...
                            RoundedRectangle(cornerRadius: Tokens.Radius.md)
                                .strokeBorder(
                                    selectedTargetID == target.id
                                        ? Tokens.actionPrimary : Tokens.line,
                                    lineWidth: selectedTargetID == target.id ? 2 : 1)
                        }
                    }
                    .buttonStyle(.plain)
                    .disabled(pendingCommand != nil)
                    .accessibilityLabel("\(target.tier), \(target.gap)단계 차이")
    ...
    private func submitButton(_ target: ServerAPI.GoatArenaMainTarget) -> some View {
        ...
        .accessibilityHint("\(target.tier)에 \(stakeDays)일을 예치하는 요청을 서버에서 확인합니다")
    }
    ...
    private func pendingNotice(_ pending: GoatArenaMainPendingCommand) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s3) {
            ...
            Text("\(pending.mode.title) · \(pending.targetTier) · \(pending.stakeDays)일. 같은 요청 번호로 결과를 다시 확인하므로 중복 생성되지 않습니다.")
                .font(.mCaption)
                .foregroundStyle(Tokens.text2)
                .fixedSize(horizontal: false, vertical: true)
    ...
    private func submit(_ target: ServerAPI.GoatArenaMainTarget) async {
        ...
        do {
            ...
            case (.invitation, "INVITATION"):
                ...
                guard owns(ownerLifecycleID, slot: ownerSlot) else { return }
                pendingCommand = nil
                statusMessage = "\(invitation.targetTier) 초대를 보냈습니다. 상대가 수락하기 전까지 아래에서 예약을 취소할 수 있습니다."
                await load()
                onInvitationCreated()
            default:
                throw GoatArenaMainSheetError.invalidReceipt
    ...
    private func userMessage(_ error: Error) -> String {
        if let server = error as? ServerAPIError {
            return ArenaDisplayTerms.apply(
                server.message ?? server.code ?? "서버 요청을 완료하지 못했습니다.")
        }
        if error is GoatArenaMainCommandStoreError {
            return "이전 요청 번호를 안전하게 보관하지 못했습니다. 저장 공간을 확인하고 다시 시도해주세요."
        }
