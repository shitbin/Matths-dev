...
struct PlacementExamScreen: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Text("화면을 나가거나 다른 앱을 사용해도 서버 제한 시간은 계속 흐릅니다. 작성한 답안은 이 iPad와 서버에 저장합니다.")
        }
        .alert(
            "요청을 완료하지 못했습니다",
            isPresented: Binding(
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: Tokens.Space.s6) {
                VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                    ...
                    Text("30문항을 100분 동안 풉니다. 앱을 벗어나도 시간은 멈추지 않으며, 답안·채점·첫 MMR은 서버가 동일한 규칙으로 처리합니다.")
    ...
    private func introMetric(_ value: String, _ label: String) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: 5) {
            Text(value).font(.mStatLarge).foregroundStyle(Tokens.ink)
    ...
    @ViewBuilder private var questionPanel: some View {
        if let question = currentQuestion {
            VStack(alignment: .leading, spacing: Tokens.Space.s6) {
                ...
                if !question.choices.isEmpty {
                    VStack(spacing: Tokens.Space.s3) {
                        ForEach(question.choices) { choice in choiceButton(choice, question: question) }
                    }
                } else {
            ...
            .padding(compact ? Tokens.Space.s5 : Tokens.Space.s6)
            .frame(maxWidth: .infinity, minHeight: compact ? 500 : 620, alignment: .topLeading)
            .background(Tokens.surface, in: RoundedRectangle(cornerRadius: Tokens.Radius.lg))
    ...
    private func resultView(_ result: Result) -> some View {
        ScrollView {
            VStack(spacing: Tokens.Space.s6) {
                RankBadgeView(tierCode: result.tierCode, size: compact ? 150 : 210, animated: true)
                VStack(spacing: Tokens.Space.s2) {
                ...
                ViewThatFits(in: .horizontal) {
                    HStack(spacing: Tokens.Space.s4) {
                        ...
                        resultMetric(result.rankPoint.map(String.init) ?? "—", "RP")
                    }
                    VStack(spacing: Tokens.Space.s3) {
                        resultMetric("\(result.totalCorrect)", "정답")
                        resultMetric(Self.scoreText(result.placementScore), "배치 점수")
    ...
    @MainActor private func saveDraft(reportFailure: Bool, closeTiming: Bool = false) async {
        ...
        do {
            ...
            guard accountSlot == DataScope.slot else { return }
            if reportFailure { actionError = "서버 저장이 늦어지고 있습니다. 답안은 이 iPad에 보관했으며 연결되면 다시 저장합니다." }
        }
    }
    ...
    private static func message(for error: Error) -> String {
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet, .networkConnectionLost:
                return "인터넷 연결이 끊겼습니다. 답안은 이 iPad에 보관됩니다."
            case .timedOut:
