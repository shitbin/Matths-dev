            ...
            ScrollView {
                ...
                ViewThatFits(in: .horizontal) {
                    if store.leftHandedOn {
                        HStack(alignment: .top, spacing: Tokens.Space.s6) { right; left }
                    } else {
                        HStack(alignment: .top, spacing: Tokens.Space.s6) { left; right }
                    }
                    VStack(spacing: Tokens.Space.s6) { left; right }
                }
                .padding(Tokens.Space.s6)
            }
            // 답안 입력 + 채점을 하단에 고정한다 — 손풀이 캔버스에 필기한 뒤
            // 답을 적으러 위로 스크롤해 돌아가는 왕복을 없앤다.
            // safeAreaInset 이라 키보드가 올라오면 바도 함께 올라온다.
            .safeAreaInset(edge: .bottom, spacing: 0) { gradeBar }
        }
    ...
    private var left: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s4) {
            VStack(alignment: .leading, spacing: Tokens.Space.s2) {
                ...
                if let p = store.currentProblem, p.isTex {
                    // 뱅크 문항 — KaTeX 발제문 + 5지선다를 웹뷰로
                    ProblemWebView(problem: p, height: $problemHeight, pickedKey: $pickedKey)
            ...
            // ▼▼▼ 디버그 채점 숏컷 — 이 묶음 하나만 주석 처리하면 통째로 사라진다 ▼▼▼
            // 검수 캡처 모드에서는 디버그 빌드라도 그리지 않는다 (R-01)
            #if DEBUG
            if !RuntimeMode.isReviewCapture {
                DebugGradeShortcut(answer: $answer)
            }
            #endif
            // ▲▲▲ 디버그 채점 숏컷 끝 ▲▲▲
        }
...
#if DEBUG
// ┌─────────────────────────── 디버그 전용 컴포넌트 ───────────────────────────┐
// │ 채점 숏컷 — 모의고사 디버깅용. 매번 답을 손으로 계산해 넣지 않아도 되게    │
// │ 정답처리 / 오답처리 두 버튼을 한 컴포지션으로 묶었다.                      │
// │                                                                            │
// │ 제거 방법: SolveScreen 안의 "디버그 채점 숏컷" 호출 묶음 하나만 주석 처리. │
// │ 릴리스 빌드에는 #if DEBUG 때문에 어차피 컴파일조차 되지 않는다.            │
// └────────────────────────────────────────────────────────────────────────────┘
struct DebugGradeShortcut: View {
    @EnvironmentObject private var store: AppStore
    @Binding var answer: String

    var body: some View {
        // 동적 문항일 때만 뜬다 — 샘플 경로는 정답 데이터가 없다
        if let p = store.currentProblem {
            HStack(spacing: Tokens.Space.s3) {
                Text("DEBUG").font(.mMicro).foregroundStyle(Tokens.text4)

                Button("정답처리") {
                    // 실제 정답을 입력란에 채우고 정규 채점 경로를 그대로 태운다
                    // (정답 대조 → 코치 onCorrect → 결과 화면까지 전부 실경로)
                    answer = p.answer
                ...
                Button("오답처리") {
                    // 어떤 문항의 정답과도 일치할 수 없는 값 — 오답 경로 강제
                    answer = "DEBUG-WRONG"
...
struct ResultScreen: View {
    ...
    private var grading: GradingResult {
        guard let grading = store.lastGrading else {
            preconditionFailure("ResultScreen content requires a real grading result")
        }
        return grading
    }

    @ViewBuilder var body: some View {
        if store.lastGrading != nil, store.currentProblem != nil {
            resultContent
        } else {
            LearningDataRecoveryView(
                ...
                VStack(alignment: .leading, spacing: Tokens.Space.s6) {
                    ...
                    HStack(spacing: Tokens.Space.s3) {
                        if !store.exam.isEmpty {
                            ...
                            Button("개념 다시 보기") {
                                store.openRelevantConceptV2(
                                    typeKey: store.currentProblem?.typeKey)
                            }
                                .buttonStyle(SecondaryButtonStyle())
                        }
                        Spacer()
                        ...
                        // (데모 목업 문구가 가드 없이 그대로 이식돼 있던 자리)
                        if grading.overall != .correct {
                            Text("오답노트에 저장됨 · 오늘 다시 복습")
                                .font(.mCaption).foregroundStyle(Tokens.text3)
                        } else {
                            Text("정답 · 오답노트에 담지 않습니다")
                                .font(.mCaption).foregroundStyle(Tokens.text3)
                        }
                    }
        ...
        VStack(alignment: .leading, spacing: Tokens.Space.s3) {
            HStack(alignment: .lastTextBaseline, spacing: Tokens.Space.s4) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(subtitle).font(.mCallout).foregroundStyle(Tokens.text2)
                        .fixedSize(horizontal: false, vertical: true)
                }
                Spacer()
                if let pts = grading.awardedPoints {
                    VStack(alignment: .trailing, spacing: 0) {
                        ...
                        (Text("\(Int(pts))").font(.mStatLarge).foregroundStyle(accent)
    ...
    @State private var solutionHeight: CGFloat = 100

    private var stepList: some View {
        VStack(alignment: .leading, spacing: 0) {
            SectionRule(title: store.currentProblem?.isTex == true ? "해설" : "단계별 채점")
                .padding(.bottom, Tokens.Space.s2)
            if let p = store.currentProblem, p.isTex {
                // 뱅크 해설은 KaTeX 한 덩어리 — 단계 분해는 서버 채점의 몫
                ProblemWebView(problem: GeneratedProblem(
                    id: p.id + "-sol", typeKey: p.typeKey, typeName: p.typeName,
                    unit: p.unit, statement: p.steps.first ?? "",
                    answer: "", steps: [], minutes: 0, isTex: true),
                    height: $solutionHeight, pickedKey: .constant(nil))
                    .frame(height: solutionHeight)
            } else {
    ...
    private var feedbackCard: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s3) {
            ...
            Text(grading.feedback).font(.mBody).foregroundStyle(Tokens.text1)
                .lineSpacing(5)
                .fixedSize(horizontal: false, vertical: true)
            // 정답을 알려주지 않는 것이 이 시스템의 계약이다
            Label("정답은 알려드리지 않습니다. 스스로 다시 풀어 보세요.",
    ...
    private func stepPickRow(_ i: Int) -> some View {
        ...
        .accessibilityLabel("\(i)단계에서 갈라짐\(picked ? ", 선택됨" : "")")
    }
        ...
        VStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Tokens.Space.s5) {
                ...
                Text(detail)
                    ...
                    .fixedSize(horizontal: false, vertical: true)
                HStack(spacing: Tokens.Space.s3) {
                    Button(primaryTitle) { store.route = primaryRoute }
                        .buttonStyle(PrimaryButtonStyle())
                    Button("홈으로") { store.route = .home }
                        .buttonStyle(SecondaryButtonStyle())
                }
            }
...
struct CoachBubble: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: Tokens.Space.s4) {
            // 마스코트 자리 — 정식 캐릭터가 나오기 전까지 불꽃으로 대신한다
            ZStack {
...
struct AssessmentScreen: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s7) {
            ...
            // ── 4. 기출 아카이브 — 3개년 수능·모평 (KiceBank). 잠금 없이 언제든 응시.
        ...
        .sheet(item: $lockDetail) { lockDetailSheet($0) }
    }
...
private struct WeeklyMockEntryCard: View {
    ...
    private var copy: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s2) {
            Text("주간 공식 모의고사")
                .font(.mMicro).foregroundStyle(Tokens.brandCyan)
...
extension AssessmentScreen {
    ...
    fileprivate var systemInfoSheet: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s5) {
            Text("평가 체계").font(.mHeading).foregroundStyle(Tokens.ink)
            VStack(alignment: .leading, spacing: Tokens.Space.s4) {
    ...
    fileprivate func lockDetailSheet(_ exam: ChainExam) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s4) {
            HStack(spacing: Tokens.Space.s2) {
                Image(systemName: "lock.fill").font(.mCaption).foregroundStyle(Tokens.text3)
                Text(exam.title).font(.mHeading).foregroundStyle(Tokens.ink)
                    .fixedSize(horizontal: false, vertical: true)
            }
...
struct WrongNotesScreen: View {
    ...
    @ViewBuilder private var reviewHero: some View {
        if !dueAll.isEmpty {
            VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                ...
                Text("약 \(dueAll.count * 4)분")
                    .font(.mCaption).foregroundStyle(Tokens.text2)
                    .monospacedDigit()
                // 복습 시작 세트 = 위의 전체 due (기존 startReview 그대로)
                Button("복습 시작") { store.startReview(ids: dueAll.map(\.id)) }
                    .buttonStyle(PrimaryButtonStyle())
                    .frame(maxWidth: 280)
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            ...
            VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                ...
                if !waitingAll.isEmpty {
                    ...
                    Button { store.startReview(ids: waitingAll.map(\.id)) } label: {
                        HStack(spacing: 2) {
                            Text("미리 복습")
                            Image(systemName: "chevron.right").font(.mMicro)
                        }
    ...
    private var emptyGhost: some View {
        VStack(spacing: Tokens.Space.s4) {
            ...
            Text("문제를 틀리면 필기 스냅샷과 함께 저장되고, 예정일에 맞춰 복습으로 돌아옵니다.")
                .font(.mCaption).foregroundStyle(Tokens.text3)
...
struct WrongNoteRow: View {
    ...
    let isOpen: Bool
    let toggle: () -> Void
    @State private var mathHeight: CGFloat = 60
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s2) {
            ...
            if isOpen {
                VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                    // 수식 있는 문항은 펼침에서 원문을 KaTeX 로 온전히 렌더
                    if MathText.containsMath(note.statement) {
                        KatexText(text: MathText.normalizeDelimiters(note.statement),
                                  height: $mathHeight)
                            .frame(height: mathHeight)
                    }
                    ...
                    // 온디바이스 AI — 진단(thinking)과 자유 질문. 모델 없으면 채팅 탭에서 안내
                    HStack(spacing: Tokens.Space.s3) {
                        Button {
                            store.route = .chat
                        ...
                        Button {
                            ...
                            Label("AI에게 묻기", systemImage: "sparkles")
                                .font(.mCaption).foregroundStyle(Tokens.text2)
                                .padding(.horizontal, Tokens.Space.s3).padding(.vertical, 6)
