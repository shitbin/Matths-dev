...
private struct WeeklyMockCenterScreen: View {
        ...
        VStack(alignment: .leading, spacing: Tokens.Space.s7) {
            ...
            if !value.eligibility.allowed {
                ...
                if !value.weeklyExams.isEmpty {
                    VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                        Text("이번 주 A · B · C").font(.mHeading).foregroundStyle(Tokens.ink)
    ...
    @ViewBuilder private var guestMetaItems: some View {
        ...
        guestMetaItem("doc.on.doc", "A·B·C 3회분")
    ...
    private func eligibilityCard(_ eligibility: ServerAPI.WeeklyMockEligibility) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s3) {
            ...
            if eligibility.status == "placement-required" || eligibility.status == "verification-required" {
                Text("GOAT Arena 탭에서 배치 절차를 먼저 완료해주세요.")
    ...
    private func examHeroCopy(_ exam: ServerAPI.WeeklyMockExamSummary) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s2) {
            Text("\(exam.formCode)형 · \(exam.attemptNumber)회차")
                .font(.mMicro).foregroundStyle(Tokens.primary)
            Text(exam.title).font(.mTitle).foregroundStyle(Tokens.ink)
    ...
    private func examRowCopy(_ exam: ServerAPI.WeeklyMockExamSummary) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            Text("\(exam.formCode)형 · \(exam.title)").font(.mBodyB).foregroundStyle(Tokens.ink)
            ...
            if value.weeklyRanking.isEmpty {
                ...
                ForEach(value.weeklyRanking.prefix(10)) { row in
                    HStack(spacing: Tokens.Space.s3) {
                        ...
                        Spacer()
                        Text(row.standardizedPerformance.map(WeeklyMockFormat.score) ?? "—")
    ...
    private func examScheduleLine(_ exam: ServerAPI.WeeklyMockExamSummary) -> String {
        ...
        return "\(release) · \(exam.questionCount)문항 · 상태 \(exam.attemptStatus)"
    }
...
private struct WeeklyMockAttemptScreen: View {
    ...
    private var attemptHeader: some View {
        VStack(spacing: 0) {
            ...
            .padding(.vertical, Tokens.Space.s2)
            if taking {
                ProgressView(value: Double(answeredCount), total: Double(max(questionCount, 1))).tint(Tokens.primary)
            }
    ...
    private var titleCopy: some View {
        VStack(alignment: .leading, spacing: 2) {
            Text("주간 공식 모의고사 · \(attempt?.exam.formCode ?? "")")
                .font(.mMicro).foregroundStyle(Tokens.primary)
            Text(attempt?.exam.title ?? "주간 공식 모의고사")
    ...
    @ViewBuilder private var timerCopy: some View {
        if taking {
            HStack(spacing: 6) {
                ...
                Text("· \(answeredCount)/\(questionCount)").foregroundStyle(Tokens.text2)
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: Tokens.Space.s6) {
                ...
                Text("\(value.exam.questionCount ?? 30)문항 · \(value.exam.durationMinutes ?? 100)분. 시작 버튼을 누르는 순간 개인 제한 시간이 시작되며, 앱을 벗어나도 서버 시계는 멈추지 않습니다.")
                    .font(.mCallout).foregroundStyle(Tokens.text2)
                VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                    Label("문제지는 시작 후 이 iPad로 안전하게 내려받습니다.", systemImage: "lock.doc.fill")
        ...
        GeometryReader { geometry in
            if geometry.size.width >= 900 {
                HStack(spacing: 0) {
                    ...
                    Divider().overlay(Tokens.line)
                    omrPane(value).frame(width: min(390, geometry.size.width * 0.36))
                }
            } else {
                VStack(spacing: 0) {
                    ...
                    .pickerStyle(.segmented).padding(.horizontal, Tokens.Space.s4).padding(.vertical, Tokens.Space.s2)
                    switch pane { case .paper: paperPane; case .omr: omrPane(value) }
                }
            }
        }
    }
...
private struct WeeklyMockResultView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Tokens.Space.s6) {
                VStack(alignment: .leading, spacing: Tokens.Space.s2) {
                    Text("SUBMITTED · \(attempt.exam.formCode)")
                        .font(.mMicro).foregroundStyle(Tokens.primary)
                ...
                if let result = attempt.result {
                    ViewThatFits(in: .horizontal) {
                        HStack(spacing: Tokens.Space.s4) { resultMetric("표준화", result.standardizedPerformance.map(WeeklyMockFormat.score) ?? "집계 중"); resultMetric("백분위", result.totalPercentile.map(WeeklyMockFormat.score) ?? "—"); resultMetric("원점수", result.rawScore.map(WeeklyMockFormat.score) ?? "공개 전") }
                        VStack(spacing: Tokens.Space.s3) { resultMetric("표준화", result.standardizedPerformance.map(WeeklyMockFormat.score) ?? "집계 중"); resultMetric("백분위", result.totalPercentile.map(WeeklyMockFormat.score) ?? "—"); resultMetric("원점수", result.rawScore.map(WeeklyMockFormat.score) ?? "공개 전") }
                    }
                }
                ...
                if let integrity = attempt.integrityReview {
                    Label("풀이과정 소명 요청이 도착했습니다. 센터의 ‘풀이과정 소명’에서 기한 안에 제출해주세요.", systemImage: "exclamationmark.shield.fill")
                        ...
                        .background(Tokens.warningSoft, in: RoundedRectangle(cornerRadius: Tokens.Radius.lg))
                        .accessibilityHint("요청 번호 \(integrity.caseId)")
                }
                ...
                if attempt.reviewAvailable == true, let review = attempt.review, !review.isEmpty {
                    VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                        ...
                        ForEach(review) { row in
                            HStack(alignment: .top, spacing: Tokens.Space.s3) {
                                ...
                                VStack(alignment: .leading, spacing: 3) {
                                    Text("\(row.number)번 · 내 답 \(row.submittedAnswer.isEmpty ? "무응답" : row.submittedAnswer) · 정답 \(row.correctAnswer)")
                                        .font(.mBodyB)
                    ...
                    Label("정답·해설은 \(WeeklyMockFormat.dateTime(attempt.reviewPublishesAt) ?? "공식 공개 시각") 이후 열립니다.", systemImage: "lock.fill")
                        .font(.mCallout).foregroundStyle(Tokens.text3)
                }
    ...
    private func resultMetric(_ label: String, _ value: String) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(value).font(.mStatLarge).foregroundStyle(Tokens.ink).minimumScaleFactor(0.65)
        }
        ...
        VStack(alignment: .leading, spacing: Tokens.Space.s4) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(selection.locked ? "선택이 마감되었습니다." : "A·B·C 중 종합 랭킹에 반영할 성적을 고릅니다.")
                        .font(.mCaption).foregroundStyle(Tokens.text3)
                }
                Spacer()
            ...
            ForEach(selection.attempts) { option in
                Button {
                    ...
                    HStack(spacing: Tokens.Space.s3) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text("표준화 \(option.standardizedPerformance.map(WeeklyMockFormat.score) ?? "—") · 백분위 \(option.totalPercentile.map(WeeklyMockFormat.score) ?? "—")")
                                .font(.mCaption).foregroundStyle(Tokens.text3)
                        }
                        Spacer()
                ...
                .buttonStyle(.plain).disabled(!selection.canChoose || busy)
            }
...
private struct WeeklyMockIntegrityScreen: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            WeeklyMockHeader(title: "풀이과정 소명", subtitle: "요청 문항 · 제출 기한 · 접수증", close: onClose)
            ScrollView {
                Group {
            ...
            if item.canSubmit {
                VStack(alignment: .leading, spacing: Tokens.Space.s3) {
                    ...
                    Text("요청 문항의 전체 풀이가 보이는 PDF 또는 이미지, 파일당 10MB 이하, 최대 10개")
                        .font(.mCaption).foregroundStyle(Tokens.text3)
                    Button { importing = true } label: { Label("파일 선택", systemImage: "paperclip") }
                        .buttonStyle(SecondaryButtonStyle())
                    ...
                    Button { Task { await submit(item) } } label: {
                        if submitting { ProgressView().tint(Tokens.onPrimary) } else { Text("소명 자료 제출") }
                    }
                    .buttonStyle(PrimaryButtonStyle()).disabled(files.isEmpty || submitting)
                }
                .card()
            }
...
private struct WeeklyMockObjectionScreen: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            WeeklyMockHeader(title: "문항 이의제기", subtitle: "공개된 시험 · 문항 단위 검토", close: onClose)
            ScrollView {
                VStack(alignment: .leading, spacing: Tokens.Space.s6) {
    ...
    private var form: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s4) {
            ...
            VStack(alignment: .leading, spacing: Tokens.Space.s2) {
                ...
                TextEditor(text: $detail).frame(minHeight: 150).padding(8)
                    .background(Tokens.paper2, in: RoundedRectangle(cornerRadius: Tokens.Radius.md))
                    .overlay(RoundedRectangle(cornerRadius: Tokens.Radius.md).strokeBorder(Tokens.line))
                Text("10자 이상, 최대 5,000자").font(.mMicro).foregroundStyle(Tokens.text4)
            }
            Button { Task { await submit() } } label: {
                if submitting { ProgressView().tint(Tokens.onPrimary) } else { Text("검토 요청 제출") }
            }
            .buttonStyle(PrimaryButtonStyle())
...
private struct WeeklyMockHeader: View {
    ...
    var body: some View {
        HStack(spacing: Tokens.Space.s4) {
            ...
            .buttonStyle(.plain).accessibilityLabel("평가센터로 돌아가기")
            VStack(alignment: .leading, spacing: 2) {
                Text(title).font(.mBodyB).foregroundStyle(Tokens.ink)
...
private enum WeeklyMockFormat {
    ...
    static func message(_ error: Error) -> String {
        ...
        if let url = error as? URLError {
            switch url.code {
            case .notConnectedToInternet, .networkConnectionLost: return "인터넷 연결이 끊겼습니다. 작성한 답안은 이 iPad에 보관됩니다."
