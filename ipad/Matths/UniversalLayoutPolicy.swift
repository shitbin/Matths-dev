...
enum UniversalLayoutPolicy {
    ...
    /// 세로가 compact인 가로모드는 탭 이름을 감춰 높이를 줄인다. VoiceOver 라벨은
    /// 뷰 계층에 그대로 남는다. 그 외 폭은 ViewThatFits가 마지막으로 판단한다.
    static func forcesIconOnlyTabs(vertical: MatthsLayoutClass) -> Bool {
        vertical == .compact
    }
