...
struct ArenaShopScreen: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Ranked 상점")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button("Arena로") { store.route = .rank }
                }
            ...
            .confirmationDialog(
                ...
                if let pendingPurchase {
                    ...
                    let base = "\(preview.beforeAvailableDays)일 → \(preview.afterAvailableDays)일 · \(duration)\n\(refund)\n잔액이 0이 되고 정산이 끝나면 Ranked에서 Unranked로 전환됩니다."
            ...
            .sheet(item: $selectedAnalysis) { effect in
    ...
    private func itemCard(_ item: ServerAPI.ArenaShop.Item, shop: ServerAPI.ArenaShop) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s4) {
            ...
            VStack(alignment: .leading, spacing: Tokens.Space.s2) {
                detailLine("잔액", "\(item.purchasePreview.beforeAvailableDays)일 → \(item.purchasePreview.afterAvailableDays)일")
            ...
            Button {
                ...
                do {
                    ...
                    errorMessage = "안전한 재시도 정보를 이 iPad에서 확인하지 못해 구매를 시작하지 않았습니다. 저장 공간을 확인한 뒤 다시 시도해 주세요."
                }
            } label: {
        ...
        if item.targetType == "MATCH" {
            ...
            Picker("가속할 초대", selection: Binding(
                ...
                ForEach(shop.invitations) { invitation in
                    Text("\(invitation.targetTier) · \(invitation.stakeDays)일 예치")
                        .tag(invitation.id)
                }
            }
            .pickerStyle(.menu)
    ...
    private func purchaseIdentity(_ purchase: ServerAPI.ArenaShop.Purchase) -> some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s1) {
            ...
            Text("\(formatDateTime(purchase.purchasedAt)) · \(purchase.status)")
                .font(.mCaption).foregroundStyle(Tokens.text3)
        }
    }

    private func purchaseBalance(_ purchase: ServerAPI.ArenaShop.Purchase) -> some View {
        Text("\(purchase.beforeAvailableDays)일 → \(purchase.afterAvailableDays)일")
            .font(.mNumeric)
            .foregroundStyle(Tokens.text2)
    }
    ...
    private func buy(_ pending: PendingPurchase) async {
        ...
        do {
            ...
            guard ownsRequest(nextID, slot: ownerSlot) else { return }
            errorMessage = "안전한 재시도 정보를 이 iPad에 저장하지 못해 요청을 보내지 않았습니다. 저장 공간을 확인한 뒤 다시 시도해 주세요."
            purchasing = false
            return
        }
        ...
        do {
            ...
            successMessage = response.receipt.replayed
                ...
                : "\(ArenaDisplayTerms.apply(response.receipt.purchase.displayName))을 적용했습니다. 잔액 \(response.receipt.beforeAvailableDays)일 → \(response.receipt.afterAvailableDays)일"
            errorMessage = nil
        } catch {
    ...
    private func applyDebugFixtureIfPresent() -> Bool {
        ...
        case "purchase":
            ...
            successMessage = "경기 상세 분석을 적용했습니다. 잔액 12일 → 11일"
        case "confirm":
...
private func arenaShopReadableError(_ error: Error) -> String {
    ...
    let message = (error as? ServerAPIError)?.message ?? error.localizedDescription
    ...
    if let apiError = error as? ServerAPIError {
        if let status = apiError.statusCode, (400..<500).contains(status) {
            ...
            return ArenaDisplayTerms.apply(
                apiError.message ?? apiError.code ?? "구매 조건을 만족하지 않아 서버가 거절했습니다."
            )
        }
