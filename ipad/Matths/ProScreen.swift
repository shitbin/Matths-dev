...
struct ProScreen: View {
    ...
    private var idleView: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s5) {
            ...
            VStack(spacing: Tokens.Space.s4) {
                ...
                if tutor.isReady && !tutor.visionAvailable {
                    // 왜 꺼졌는지를 **사실대로** 갈라 말한다. 파일이 있는데도
                    // "모듈이 없다" 고 하면 사용자는 고칠 길이 없다.
                    Text(ModelDownloader.visionOffForCurrentModel
                         ? "지난 실행에서 사진 분석 모듈을 여는 도중 앱이 종료돼 자동으로 꺼 두었습니다. 아래 버튼으로 다시 시도할 수 있습니다."
                         : (ModelDownloader.mmprojFilePresent
                            ? "사진 분석 모듈은 있지만 이 조합(메모리)에서는 열지 않습니다. "
        ...
        #if DEBUG
        .sheet(isPresented: $showLog) { GraderLogScreen() }
        #endif
        ...
        .fileImporter(isPresented: $showFileImporter,
            ...
            case .failure(let e):
                finishPick(nil, "파일을 열지 못했습니다: \(e.localizedDescription)")
            }
        }
    ...
    @ViewBuilder private var photoSourceButtons: some View {
        ...
        Menu {
            ...
            Label("보관함·파일에서 가져오기", systemImage: "photo.on.rectangle")
    ...
    @MainActor private func finishPick(_ img: UIImage?, _ message: String?) {
        ...
        catch { errorText = "사진을 저장하지 못했습니다: \(error.localizedDescription)"; return }
        shotPath = url.path
        grader.stop()
    ...
    private func startOnDeviceAnalysis() {
        ...
        do {
            ...
            errorText = "분석 사진을 안전하게 보관하지 못했습니다: \(error.localizedDescription)"
            return
        }
        stage = .scanning
    ...
    private func tryStartIfReady() {
        ...
        analysisPreparationTask = Task {
            ...
            do {
                ...
                if analysisPreparationID == preparationID { errorText = error.localizedDescription }
            }
            if let workLease, !handedLeaseToGrader {
                await LocalAIWorkCoordinator.shared.release(workLease)
            }
    ...
    private var onDeviceScanningView: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack {
                Text("기기 안에서 처리 중 — 사진은 서버로 가지 않습니다")
                    .font(.mCaption).foregroundStyle(Tokens.text3)
                Spacer()
                Button("중단") {
                    grader.stop()
            ...
            Text("다른 앱을 열면 iPadOS가 허용하는 짧은 시간 동안만 이어집니다. 중단되거나 앱이 종료돼도 이 사진을 보존해 다음 실행에서 처음부터 다시 시작할 수 있습니다.")
                .font(.mMicro)
            ...
            VStack(alignment: .leading, spacing: Tokens.Space.s4) {
                ...
                if wrongTypes.isEmpty {
                    ...
                    Text("이 유형만으로 새 모의고사를 만듭니다. 수치와 정답은 회차마다 다시 뽑히므로 "
                         + "같은 문제를 외워서 넘어갈 수 없습니다. 생성은 기기 안에서 끝납니다 (AI·서버 불필요).")
                        .font(.mCaption).foregroundStyle(Tokens.text3)
                        .lineSpacing(4)
                        .fixedSize(horizontal: false, vertical: true)
