...
//  좁은 폭(Split View·세로)에서는 문제지/답안지 세그먼트 전환으로 내려간다.
//
//  채점 규약:
...
struct KiceExamScreen: View {
        ...
        VStack(spacing: 0) {
            ...
            if let exam = store.kiceExam {
                GeometryReader { geo in
                    // 900pt 이상이면 문제지·답안지를 나란히. 그 아래는 세그먼트 전환.
                    // OMR 폭은 고정 360 대신 전체의 32% — 다만 5지선다 한 행이
                    // 물리적으로 요구하는 344pt 아래로는 내리지 않는다.
                    if geo.size.width >= 900 {
                        let omrWidth = min(max(geo.size.width * 0.32, 344), 480)
                        HStack(spacing: 0) {
                            pdfPane(exam)
                ...
                // 인덱스가 없으면 여기까지 올 수 없지만, 죽는 대신 돌아갈 길을 준다
                ContentUnavailableView("기출 데이터를 찾을 수 없습니다",
    ...
    private var timeStage: TimeStage {
        ...
        let remaining = Self.examLengthMs - timer.elapsedMs
    ...
    private var timeNotice: String? {
        ...
        case .critical:
            return Self.examLengthMs - timer.elapsedMs <= 0
                ...
                ForEach(1...5, id: \.self) { n in
                    ...
                    Button {
                        ...
                        Text(Self.circled[n - 1])
                            ...
                            // 원은 34pt 로 두고 히트 영역만 세로 44pt 로 늘린다
