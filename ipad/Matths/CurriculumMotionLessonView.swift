...
struct CurriculumMotionLessonView: View {
    ...
    private var checkCard: some View {
        VStack(alignment: .leading, spacing: Tokens.Space.s3) {
            ...
            if explanationMode == "correct" {
                Button(sceneIndex < scenes.count - 1 ? "다음 장면" : "아래 연습 문제로") {
                    if sceneIndex < scenes.count - 1 {
                        selectScene(sceneIndex + 1)
                    } else {
                        explanation = "이제 바로 아래 연습 문제에서 같은 관계를 사용해 보세요."
                    }
                }
                .buttonStyle(PrimaryButtonStyle())
                .frame(maxWidth: .infinity, minHeight: 48)
    ...
    private func selectChoice(_ index: Int) {
        if index == current.answerIndex {
            ...
            explanation = current.correctFeedback
        } else {
            misses += 1
