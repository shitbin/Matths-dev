...
struct WrongNoteEntry: Codable, Identifiable {
    let id: String
...
enum WrongNoteDisk {
    ...
    static func load() -> [WrongNoteEntry] {
        ...
        if let bak = try? Data(contentsOf: backupURL), let list = decode(bak) {
            WrongNoteStorageAlertCenter.shared.raise(
                ...
                + (preserved != nil ? " 손상된 원본은 \(preserved!) 로 보관했습니다." : ""))
        ...
        WrongNoteStorageAlertCenter.shared.raise(
            ...
            + (preserved != nil ? " 원본은 \(preserved!) 로 보관했습니다." : "")
    ...
    static func save(_ list: [WrongNoteEntry]) -> Bool {
        ...
        do {
            ...
            WrongNoteStorageAlertCenter.shared.raise(
                "오답노트를 저장하지 못했습니다 — 저장 공간을 확인해 주세요. (\(error.localizedDescription))")
            return false
        }
    }
