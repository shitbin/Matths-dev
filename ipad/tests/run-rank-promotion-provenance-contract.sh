...
node -e 'const fs=require("fs");const p=process.argv[1];const j=JSON.parse(fs.readFileSync(p));j.sourceProvenance.approvedSource=true;fs.writeFileSync(p,JSON.stringify(j));' \
  "$app/RankMotion/rank-promotion-assets.json"
if node "$root/scripts/verifyRankPromotionBundle.js" \
  --app "$app" --source-root "$source_root" >/dev/null 2>&1; then
  echo '승인으로 과장한 rank manifest가 provenance 검증을 통과했습니다.' >&2
  exit 1
fi
