...
- 운영 승인된 리디렉션 URI: `https://matths.kr/auth/google/callback`
- 범위: `openid`, `email`, `profile`

`config.env`에 다음 값을 입력한다.
...
- 운영 도메인의 HTTPS 콜백 URI와 `config.env` 값이 글자 단위로 일치해야 한다.
- 비밀 키는 Git에 커밋하지 않는다.
...
- 로컬 전용 환경에서는 callback을 `http://localhost:8000/auth/google/callback`으로 덮어쓸 수
  있지만 Cafe24 운영 설정과 Google Console 운영 URI는 반드시 `https://matths.kr`로 고정한다.
