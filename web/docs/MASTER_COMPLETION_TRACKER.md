...
| 7 | iPad Google 로그인 실기 | 구현 | 웹 OAuth의 운영 정본 callback은 `https://www.matths.kr/auth/google/callback`이다. 구형 Cafe24 운영본의 iPad `/api/v1/auth/google/start`가 401을 반환한 이력이 있어 최신 후보는 공개 start/exchange를 Bearer 미들웨어 앞에 두고 동일 서버 OAuth·단일 사용 코드·callback 검증·공식 4색 마크를 구현했다. Cafe24 후보 반영 후 기존·신규 계정·취소·재시도·앱 복귀 실기 통과가 남아 있다. |
